...
function simulate_classical_failure()
  # Parameters
  screen_length     = 0.974  
  steric_core       = 0.137  
  A_coulomb         = 3.085  
  ...
  lambda_smearing   = 0.005 # Near-zero limit to prevent division by zero
  ...
  p = plot(time_axis, history, legend=false, linewidth=1.5, palette=:Reds_9, bg_inside=:whitesmoke)
  title!("Classical Limit: Zero-Flux Gridlock")
  xlabel!("Dimensionless Time (t)")
  ylabel!("Dimensionless Position (x)")
  annotate!(time_axis[15000], 8.5, text("Trapped at Steric Bottleneck", :red, 10, :center))
  hline!([6.0], color=:black, linestyle=:dash, linewidth=2, label="Bottleneck")

  plot!(size=(800, 600), dpi=300, margin=5Plots.mm)
  display(p)
  savefig(p, "classical_gridlock.png")
end

simulate_classical_failure()
